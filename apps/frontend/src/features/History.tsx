...
function Evolution({ turns, axis, who }: { turns: Turn[]; axis: Axis; who: string }) {
  ...
  const points = turns.map((turn, i) => {
    ...
    return { turn, value, x: turns.length === 1 ? 320 : 40 + i * 560 / (turns.length - 1), y: value === undefined || value < 0 || value > 10 ? undefined : 170 - value * 14 };
  });
  let connected = false;
